...
```{r include plain X by Y, echo=FALSE, warning=FALSE}
include_graphics("https://github.com/mmwilmes/Linear_regression_foundation/raw/master/img/X_versus_Y_plain.jpg") 
```
...
```{r plot with regression line, echo=FALSE, out.width='1200px', dpi=100}
dat %>%
  ggplot() +
  aes(x, y) +
    ylab("Response (Y)") +
  ...
  theme_bw()
ggsave("./img/X_versus_Y_with_line.jpg", dpi = 320,  width = 5, height = 3, units = c("in"))
```

```{r include Y~X with line, echo=FALSE, warning=FALSE}
# include_graphics("https://github.com/mmwilmes/Linear_regression_foundation/raw/master/img/X_versus_Y_with_line.jpg") 
```


<!-- wp:paragraph -->
Mathematically, such a regression line is described as $\hat{Y_i} = \beta_0 + \beta_1 X_i + \epsilon_i$.
...
```{r plot with regression line and residuals, echo=FALSE, out.width='1200px', dpi=100}
dat %>%
  ggplot() +
  aes(x, y) +
    ylab("Response (Y)") +
  ...
  geom_point() +
  ggtitle("Relationship of X and Y with residuals") +
  theme_bw()
ggsave("./img/X_versus_Y_with_line_and_res.jpg", dpi = 320,  width = 5, height = 3, units = c("in"))
```

```{r include Y~X with line and res, echo=FALSE, warning=FALSE}
# include_graphics("https://github.com/mmwilmes/Linear_regression_foundation/raw/master/img/X_versus_Y_with_line_and_res.jpg") 
```
...
<!-- wp:paragraph -->
Once your algorithm optimized according to the Loess function and identified the best-fit model, we can retrieve $\beta_0$ and $\beta_1$.
<!-- /wp:paragraph -->

```{r plot with residuals; regression line; and formula, echo=FALSE, out.width='1200px', dpi=100}
dat %>%
  ggplot() +
  aes(x, y) +
    ylab("Response (Y)") +
...
```{r include Y~X with line res and formula, echo=FALSE, warning=FALSE}
# include_graphics("https://github.com/mmwilmes/Linear_regression_foundation/raw/master/img/X_versus_Y_with_line_res_formula.jpg") 
```
...
<!-- wp:paragraph -->
This brings us to the end of the linear regression basics. In the next post, we'll look at the assumptions that underly linear regression, see how to test them, learn when it's (not) OK to ignore them, and how to transform your data to make them comply to the linear regression assumptions.
<!-- /wp:paragraph -->
